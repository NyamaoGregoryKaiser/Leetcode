...
/**
 * CustomMaxHeap: An array-based implementation of a Max-Heap.
 * A Max-Heap is a complete binary tree where the value of each node
 * is greater than or equal to the value of its children. The largest
...
public class CustomMaxHeap<T> {
    ...
    /**
     * Constructs an empty CustomMaxHeap using natural ordering.
    ...
    /**
     * Constructs an empty CustomMaxHeap with a custom comparator.
     *
     ...
     * For a Max-Heap, we want the larger element to bubble up.
     *
    ...
    private int compare(T a, T b) {
        if (comparator != null) {
            ...
            // For max-heap, we usually want greater elements to have higher priority.
            // The comparator defines natural ordering. If we want reverse for max-heap,
            // we could reverse its result, but usually a max-heap comparator naturally
            // returns positive if a > b. Here, we'll just use it directly for simplicity
            // and assume it's configured for MaxHeap if needed (e.g. Collections.reverseOrder()).
            return comparator.compare(a, b);
        } else {
            // For natural ordering, compareTo returns positive if 'a' is greater than 'b'.
            // This is exactly what we need for a Max-Heap's comparison logic.
    ...
    /**
     * Retrieves (but does not remove) the maximum element in the heap.
    ...
    public T peek() {
        ...
        return heap.get(0); // The root is always the maximum
    }
     ...
     * them if the parent is smaller (for a max-heap), continuing until the
     * heap property is satisfied or the root is reached.
     * Time Complexity: O(log N)
     */
    private void heapifyUp() {
        int currentIndex = heap.size() - 1; // Start from the last element
        // For a max-heap, we bubble up if current element is greater than parent
     ...
     * it with the larger child (for a max-heap), continuing until the heap
    ...
    private void heapifyDown() {
        ...
        while (hasLeftChild(currentIndex)) {
            ...
            if (hasRightChild(currentIndex) && compare(heap.get(getRightChildIndex(currentIndex)), heap.get(largerChildIndex)) > 0) {
                largerChildIndex = getRightChildIndex(currentIndex);
            }

            // If the current element is larger than or equal to its largest child,
            // heap property is satisfied.
            if (compare(heap.get(currentIndex), heap.get(largerChildIndex)) >= 0) {
                break;
            } else {
                // Otherwise, swap with the larger child and continue down
