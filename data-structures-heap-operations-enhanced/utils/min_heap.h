...
/**
 * @brief Custom Min-Heap implementation using a vector.
 *
 * This class provides a basic Min-Heap data structure with common operations
...
private:
    ...
    /**
     * @brief Restores the Min-Heap property by moving an element up the heap.
     *        This is called after an insertion.
     * @param idx The index of the newly inserted element.
     */
    void heapify_up(int idx) {
        // While current node is not root (idx > 0) AND current node is smaller than its parent
    ...
    /**
     * @brief Restores the Min-Heap property by moving an element down the heap.
    ...
    void heapify_down(int idx) {
        ...
        // If smallest is not current node, swap and continue heapifying down
        if (smallest != idx) {
            std::swap(data[idx], data[smallest]);
            heapify_down(smallest); // Recursively call for the swapped child
        }
    }
...
public:
     ...
     */
    MinHeap(const std::vector<T>& arr) : data(arr) {
    ...
    /**
     * @brief Returns a const reference to the minimum element (root) of the Min-Heap.
...
// Specialization for pairs to enable MinHeap on custom criteria
// Example: MinHeap<std::pair<int, int>> will by default compare first element
// If you need custom comparison, pass a custom comparator to std::priority_queue
// or modify the comparison logic in heapify_up/down for custom MinHeap.
// For this project, default pair comparison (first then second) is fine for `Top K Frequent`.
// For `Merge K Sorted Lists`, we need to compare `ListNode*` by their `val`. This will require
// a custom comparator struct to be passed or a dedicated `MinHeap<ListNode*>` where the comparison
// is hardcoded, or the ListNode* itself implements operator< (less ideal).
// Let's assume for ListNode* comparisons will be handled by a custom comparator or the problem setup.
// For the general MinHeap class, standard `operator<` for T is assumed.